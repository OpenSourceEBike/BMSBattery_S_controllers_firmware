...
uint16_t update_setpoint (uint16_t speed, uint16_t PAS, uint16_t sumtorque, uint16_t setpoint_old)
{
  ...
  if (ui32_SPEED_km_h>limit && setpoint_old>(ui32_SPEED_km_h-limit)){ui32_setpoint=(uint32_t)setpoint_old-(ui32_SPEED_km_h-limit);} 			//reduce (old) setpoint, if you are riding too fast
  ...
  printf("nach: spd %d, pas %d, sumtor %d, setpoint %lu, km/h %lu\n", speed, PAS, sumtorque, ui32_setpoint, ui32_SPEED_km_h);
